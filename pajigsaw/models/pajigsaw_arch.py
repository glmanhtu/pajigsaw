...
from pajigsaw.utils.utils import trunc_normal_
...
class CrossAttention(nn.Module):
    ...
    def forward(self, x, context):
        ...
        # N - 1 ==> context doesn't have cls token
        kv = self.kv(context).reshape(B, N - 1, 2, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        k, v = kv[0], kv[1]
...
class CrossBlock(nn.Module):
    ...
    def forward(self, x, context, return_attention=False):
        ...
        y, attn = self.cross_attn(self.norm_cross(x), self.norm_context(context))
        if return_attention:
            return attn
        x = x + self.drop_path(y)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
...
class PaJigSaw(nn.Module):
    """ Vision Transformer """
    def __init__(self, img_size=[224], patch_size=16, in_chans=3, num_classes=0, embed_dim=768, depth=6,
                 cross_depth=6,
