...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, images):
        sizes = [len(images), len(images)]
        B, IM, C, H, W = images.shape
        x = images.view(-1, C, H, W)
        x = self.backbone.forward_features(x)
        x1, x2 = torch.split_with_sizes(x, sizes, dim=0)
...
def pajigsaw_base(patch_size=16, img_size=224, num_classes=1, use_abs_pos=False, use_rel_pos=True, **kwargs):
    model = VisionTransformer(
        ...
        cross_depth=6, num_heads=12, mlp_ratio=4, qkv_bias=True, drop_rate=0.0,
