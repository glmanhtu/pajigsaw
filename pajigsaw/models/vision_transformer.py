...
class CrossAttention(nn.Module):
    def __init__( self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0.,
        ...
        self.kv = nn.Linear(dim, all_head_dim * 2, bias=qkv_bias)
    ...
    def forward(self, x, context, rel_pos_bias=None):
        ...
        kv = self.kv(context).reshape(B, N, 2, self.num_heads, -1).permute(2, 0, 3, 1, 4)
...
class CrossBlock(nn.Module):
    ...
    def forward(self, x, context, rel_pos_bias=None):
        if self.gamma_1 is None:
            ...
            x = x + self.drop_path(self.cross_attn(self.norm_cross(x), self.norm_context(context),
                                                   rel_pos_bias=rel_pos_bias))
            x = x + self.drop_path(self.mlp(self.norm2(x)))
        else:
            x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x), rel_pos_bias=rel_pos_bias))
            x = x + self.drop_path(self.gamma_2 * self.cross_attn(self.norm_cross(x), self.norm_context(context),
                                                                  rel_pos_bias=rel_pos_bias))
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, images):
        ...
        for blk in self.blocks:
            x1 = blk(x1, rel_pos_bias=rel_pos_bias)

        for blk in self.cross_blocks:
            x2 = blk(x2, x1, rel_pos_bias=rel_pos_bias)
...
def pajigsaw_base(patch_size=16, img_size=224, num_classes=1, use_abs_pos=False, use_rel_pos=True, **kwargs):
    model = VisionTransformer(
        img_size=img_size, patch_size=patch_size, in_chans=3, num_classes=num_classes, embed_dim=768, depth=12,
        cross_depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, drop_rate=0.0,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_abs_pos_emb=use_abs_pos,
        use_rel_pos_bias=use_rel_pos, use_shared_rel_pos_bias=False, use_mean_pooling=True, **kwargs)
    return model


def pajigsaw_small(patch_size=16, img_size=224, num_classes=1, use_abs_pos=False, use_rel_pos=True, **kwargs):
    model = VisionTransformer(
        img_size=img_size, patch_size=patch_size, in_chans=3, num_classes=num_classes, embed_dim=384, depth=12,
        cross_depth=12, num_heads=6, mlp_ratio=4, qkv_bias=True, drop_rate=0.0,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), use_abs_pos_emb=use_abs_pos,
        use_rel_pos_bias=use_rel_pos, use_shared_rel_pos_bias=False, use_mean_pooling=True, **kwargs)
    return model
